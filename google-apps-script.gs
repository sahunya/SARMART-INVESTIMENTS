...
const HEADERS = ['id', 'type', 'name', 'category', 'party', 'amount', 'paidAmount', 'status', 'date', 'due', 'notes', 'createdAt'];
...
function createRecord_(record) {
  if (!record || !record.id || !record.type || !record.amount || !record.date) throw new Error('Missing transaction details.');
...
function updateRecord_(record) {
  ...
  sheet.getRange(index + 2, 1, 1, headers.length).setValues([headers.map(header => values[header] ?? '')]);
  return record;
}
